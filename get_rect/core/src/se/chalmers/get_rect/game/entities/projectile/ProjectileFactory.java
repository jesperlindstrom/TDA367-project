...
public class ProjectileFactory {
    ...
    public IPhysicsEntity make(String type, Point point, Point velocity){
        if (type.equals("normal"))
            return makeProjectile(point, velocity);

        if (type.equals("cluster"))
            return makeClusterProjectile(point, velocity);

        throw new EntityNotFoundException("Projectile", type);
    }

    private IPhysicsEntity makeProjectile(Point point, Point velocity) {
    ...
    private IPhysicsEntity makeClusterProjectile(Point point, Point velocity) {
        Projectile model = new Projectile(point, velocity, rectangleFactory, this);
        IView view = new ProjectileView(model);
